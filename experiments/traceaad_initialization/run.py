"""Bounded initialization comparison using the unchanged V10.13 engine."""
import argparse
import json
import re
import traceback
...
class InitializationExperiment(TraceAADV1013):
    ...
    def _schedule(self):
        if len(self.tree.roots) < self.n_roots:
            ...
            prompt = builder.build_initial().prompt
            metadata = {'candidate_id': self.candidate_count + 1,
                ...
                'init_context_ids': [int(x) for x in re.findall(
        ...
        try:
            while self.evaluations_used < self.budget and self.candidate_count < self.max_calls:
                if self.init_only and len(self.tree.roots) >= self.n_roots:
                    break
                self._run_candidate()
                print(f'{self.init_mode}: E{self.evaluations_used}, roots={len(self.tree.roots)}', flush=True)
            status = 'finished' if len(self.tree.roots) >= self.n_roots else 'incomplete_initialization'
            if not self.init_only and self.evaluations_used < self.budget:
                status = 'call_cap'
...
def main():
    ...
    params = dict(budget=args.budget, n_roots=8, history_depth=3,
        ...
        init_mode=args.init_mode, init_only=args.init_only, max_calls=80)
    ctx = setup_experiment_run(args, method='initialization_v1013',
        method_dir=Path(__file__).parent, resume_file='tree_state.json', method_params=params)
    try:
