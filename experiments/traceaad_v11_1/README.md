...
```bash
uv run python -m experiments.traceaad_v11_1.manual_launch \
...
监控入口为 `uv run python -m core.training_monitor --port 8765`，默认只读取 V11.1 的 manifest、结果与 tmux 状态；旧的 `experiments.traceaad_v11_1.monitor` 仍保留为兼容入口。面板优先展示评价预算、Best 改进轨迹、父代/全局前沿改善和候选血统，耗时与 token 只作诊断。统一调度器及 V11.1 旧自动 launcher 已移除。
