...
Features:
- Full overview and run inspection for V11.0 experiments
...
- Real-time progress across all 25 runs (5 tasks x 5 repeats)
...
class MonitorDataEngine:
    ...
    def get_available_versions(self) -> list[dict[str, Any]]:
        ...
        for manifest in _load_batch_manifests(self.default_results_root):
            ...
            versions.append(
                {
                    "id": batch,
                    "name": f"TraceAAD V11.0 · {batch}",
                    "badge": batch,
                    "is_latest": not versions,
                }
            )
        if not versions:
            fallback = self.default_version or "v110"
            versions.append(
                {
                    "id": fallback,
    ...
    def _resolve_version_meta(self, version: str | None = None) -> tuple[str, Path, str, str]:
        ...
        if chosen is not None:
            batch = chosen["batch"]
            return batch, self.default_results_root, chosen.get("session_prefix") or "v110", batch
        fallback = want or "v110"
        return fallback, self.default_results_root, self.default_session_prefix or "v110", fallback.upper()
