...
PORT="${1:-8766}"
SESSION="traceaad_monitor_v110"

if tmux has-session -t "${SESSION}" 2>/dev/null; then tmux kill-session -t "${SESSION}"; fi
cd "${ROOT}"
CMD="${ROOT}/.venv/bin/python -u -m core.training_monitor --results-dir ${ROOT}/experiments/traceaad_v11_0/results --host 0.0.0.0 --port ${PORT}"
