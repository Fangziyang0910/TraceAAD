...
```bash
uv run python -m experiments.monitor \
  --host 127.0.0.1 --port 8766 --experiment traceaad_v10_13
```

页面显示批次总进度、五个任务的各次重复，以及单路最佳值曲线、近期候选和最佳代码。每路默认 E1000。一次评价器调用计一次预算，解析失败不计；原始记录保存在 `experiments_result/traceaad_v10_13/`。
