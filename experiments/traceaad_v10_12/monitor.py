"""Live V10.12 training monitor using batch manifests, results and tmux sessions.

Usage:
    python -m experiments.traceaad_v10_12.monitor [--port 8765] [--host 0.0.0.0]
"""
...
DEFAULT_RESULTS_ROOT = REPO_ROOT / "experiments/traceaad_v10_12/results"
...
class MonitorDataEngine:
    ...
    def get_available_versions(self) -> list[dict[str, Any]]:
        ...
        for root_dir, manifest in manifests:
            ...
            if not method:
                label = str(batch)
            elif method.startswith("v1012"):
                label = "TraceAAD V10.12"
        ...
        if not versions:
            fallback = self.default_version or "v1012"
            versions.append(
                {
                    "id": fallback,
    ...
    def _resolve_version_meta(self, version: str | None = None) -> tuple[str, Path, str, str]:
        ...
        if chosen is not None:
            ...
            method = manifest.get("method")
            if not method:
                return batch, root_dir, manifest.get("session_prefix") or "v1012", batch
            elif method.startswith("v1012"):
                label = "V10.12"
            elif method.startswith("v110"):
                label = "V11.0"
                ...
                label = str(method)
            return batch, root_dir, manifest.get("session_prefix") or "v1012", f"{label} · {batch}"
        fallback = want or "v1012"
        return fallback, self.default_results_root, self.default_session_prefix or "v1012", fallback.upper()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Single results directory override (defaults to V10.12 results)",
    )
    parser.add_argument(
        "--results-dirs",
        type=Path,
